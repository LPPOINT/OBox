...
{
    public class LSIcon : MonoBehaviour
    {
        ...
        {
            SetScale(Model.Type == LSIconModel.IconType.CurrentLevel ? CurrentLevelScale : 1);

            if(Model.Type == LSIconModel.IconType.CurrentLevel)
                StartScaleAnimation();
            else 
                StopScaleAnimation();

            if (Model.Type == LSIconModel.IconType.LockedLevel)
            {
        ...
        {
            if(Model.Type != LSIconModel.IconType.LockedLevel)
                GUISupervisor.Instance.OpenLevel((int)Model.Number, Model.World);
        }
    }
}
